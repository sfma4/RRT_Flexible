...
class RRTGraph(object):
    ...
    def __init__(self, *nodes):
        ...
        self.f_nodes = [n for n in nodes]  ## building a nodes list for pesudo goal only
        ...
        self.pos_tip = defaultdict(lambda: None)
        self.state_type = defaultdict(lambda: None)

    def add_node(self, new_config, parent, path, pos_tip, x_y, theta, type_num):
        ...
        self.pos_tip[new_config] = pos_tip
        self.x_y[new_config] = x_y
        self.joints[new_config] = theta
        self.state_type[new_config] = type_num

    # def add_f_node(self, new_config, parent, path, pos_tip, x_y, theta, type_num):
...
class RRTPlanner(object):
    ...
    def dis(self, c1, c2):
        ...
        0.6 * ((sin_theta1 - sin_theta2) ** 2 + (cos_theta1 - cos_theta2) ** 2)
        return result

    def distance(self, c1, c2, config_list, graph, sample_type):
        x1 = c1[0]
        y1 = c1[1]
        a1 = c1[2]
        b1 = c2[3]
        ...
        end_tip = [x1 + 0.5*np.sin(a1) + 0.5*np.sin(b1), y1 - 0.5* np.cos(a1) - 0.5*np.cos(b1)]
        ### end_tip is the final location

        pena_2 = 0.0
        for i in range(len(config_list)):
            # print('config list', config_list)
            if graph.state_type[config_list[i]] == 2:
                print('distance as denominator', self.dis(config_list[i], c2), self.dis(self.graph.parent[config_list[i]], c1))
                pena_2 = pena_2 + 20/(self.dis(config_list[i], c2) + self.dis(self.graph.parent[config_list[i]], c1))
        ### if the iterated sample in the list is a failed pseudo goal, then a penalty is added based on the proximity
        # between c2 and the iterated sample and between c1 and the parent node of the iterated sample
        pena_1 = 0.0
        print('c1 state', self.graph.state_type[c1])

        # if self.graph.state_type[c1] == 0 or self.graph.state_type[c1] == 2:
        if c1[-1] == 0 or c1[-1] == 2:
            pena_1 = 1000.0
        print('pena_2', pena_2)
        result = self.dis(c1, c2) + pena_2 + pena_1
        print('distance result', result)

        return result

    def nearest_config_to(self, config_list, config, graph, sample_type):
        """
            Finds the configuration from config_list that is closest to config.
        """
        print('===make sure using the correct nearest function111===')
        return np.asarray(min(config_list, key=lambda c: self.distance(c, config, config_list, graph, sample_type)))
    ...
    def get_alpha(self, goal):    ### to do
        ...
        a = np.array([goal[0], goal[1],alpha[0][0], beta, 1])
        b = np.array([goal[0], goal[1],alpha[0][0], -beta, 1])
    ...
    def plan_to_pose(self, start, goal, dt=0.01, prefix_time_length=1):
        ...
        goal_a = np.array([6.6, 5.6, 0.8, 0.2, 1])
        ...
        self.graph.state_type[tuple(goal_a)] = 1
        self.plan = None
        t_start = time.time()
        for i in range(40):
            ...
            if sample_type == 0 :
                ...
                new_goal_p = np.array([x[0][0], y[0][0], 0.0, 0.0, 0]) ##now we try without the help of steady state

                closest_config_p = self.nearest_config_to(self.graph.nodes, new_goal_p, self.graph, sample_type)
                ...
                if np.allclose(end_pos_p[:2], end_pos_closest, rtol=tol, atol=tol) \
                    ...
                    self.graph.add_node(new_goal_p, closest_config_p, path_p, end_pos_p, x_y, theta, 0)
                    ...
                    if np.allclose(end_pos_p_2[:2], [7.4, 5.1], rtol=0.05, atol=0.05):
                        ...
                        parent = [('none')]
                        result = [('none')]
                        node_type = [('none')]## because for 1st point, there is no parent or result
                        for i in range(len(nodes)-1):
                            p = self.graph.parent[self.graph.nodes[i+1]]
                            parent.append(p)
                            r = self.graph.pos_tip[self.graph.nodes[i+1]]
                            result.append(r)
                            t = self.graph.state_type[self.graph.nodes[i + 1]]
                            node_type.append(t)

                        df = pd.concat(
                            [pd.DataFrame({'nodes': nodes}), pd.DataFrame({'parent': parent}),
                             pd.DataFrame({'result': result}), pd.DataFrame({'type': node_type})],
                            axis=1)
                        ...
                        df = pd.concat(
                            ...
                            axis=1)
                        timenow = time.strftime("%Y%m%d-%H%M%S")
                        df.to_csv(timenow + '_failed_result.csv')
                        self.graph.delete_node(new_goal_p)
                        self.graph.add_node(new_goal_p, closest_config_p, path_p, end_pos_p, x_y, theta, 2)
                        ### the last entry of add_node is 2, represents it is a failed connection
                else:
                    new_goal_p[-1] = 2
                    self.graph.add_node(new_goal_p, closest_config_p, path_p, end_pos_p, x_y, theta, 2)
            ...
            if sample_type == 1:
                ...
                closest_config_1 = self.nearest_config_to(self.graph.nodes, sample_1, self.graph, sample_type) #found the nearest point from the graph.nodes
                closest_config_2 = self.nearest_config_to(self.graph.nodes, sample_2, self.graph, sample_type)
                ...
                if closest_config_1[2]*sample_1[2] >= 0 and closest_config_1[3]*sample_1[3] >= 0:
                    ...
                    if np.allclose(end_pos_1[:2], end_pos_closest_1, rtol=tol, atol=tol):
                        ...
                        self.graph.add_node(sample_1, closest_config_1, path_final_1, end_pos_1, x_y, theta, 1)
                if closest_config_2[2] * sample_2[2] > 0 and closest_config_2[3]*sample_2[3] >= 0:
                    ...
                    if np.allclose(end_pos_2[:2], closest_config_2[:2], rtol=0.1, atol=0.1):
                        ...
                        self.graph.add_node(sample_2 , closest_config_2, path_final_2, end_pos_2, x_y, theta, 1)
                        ### TODO TODO TODO TODO
        ...
        # dict = {'nodes': nodes, 'parent': parent, 'result':result}
        df = pd.concat(
            [pd.DataFrame({'nodes': nodes}), pd.DataFrame({'parent': parent}), pd.DataFrame({'result': result}), pd.DataFrame({'type': node_type})],
            axis=1)
        timenow = time.strftime("%Y%m%d-%H%M%S")
        df.to_csv(timenow + '_failed_finding_path.csv')
