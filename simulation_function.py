...
def simulation_try(p1, alpha_initial):
    ...
    p.resetDebugVisualizerCamera(cameraDistance=12.5, cameraYaw=36.0, cameraPitch=-30,
    ...
    p.changeDynamics(boxId, 3, linearDamping=1.0, angularDamping=10.0)
    p.changeDynamics(boxId, 4, lateralFriction=0.25)
    print('dynamics', p.getDynamicsInfo(boxId, 4))

    print('underactuated joint dynamics', p.getJointInfo(boxId,3))
    target_v = 5.0  # motor angular speed rad/s  @@@@@@ how come this value change the position result?
    # target_v = [target_v, target_v]
    max_force = 80  # max force exerted by motor
    x_plot, y_plot = [], []
    force = []
    joint1_pos = []
